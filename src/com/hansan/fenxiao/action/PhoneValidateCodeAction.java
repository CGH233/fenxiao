...
public class PhoneValidateCodeAction extends BaseAction
{
  ...
  public void createCode()
  {
    ...
    if (findUser == null) {
      ...
      int code = random.nextInt(99999) + 100000;//随机生成6位验证码
      ...
      try {
        SendSms.send(smsContent, phone);//发送短信验证码
      } catch (IOException e) {
        json.put("status", "0");
  ...
  public void validationCode() {
    ...
    if (findPhoneValidateCode == null) {
      ...
      if (nowTime > codeTime + 600000L) {
        ...
        try {
          this.request.getRequestDispatcher("findPasswordNext.jsp").forward(this.request, this.response);//请求转发到findPasswordNext.jsp
